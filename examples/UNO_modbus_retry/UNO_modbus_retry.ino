...
void setup() {
  ...
  Serial.println(" reading Slave ID: 1, Holding Register 0, Qty 4 at 9600baud every 3secs." );
  Serial.println("  will retry cmd once if it times out.");
...
void loop() {
  ...
  if (samplingDelay.justFinished()) {
    ...
    if (nbModbusMaster.readHoldingRegisters(slaveId, address, qty)) {
      // cmd started
      start_ms = millis();
    } else { // else still waiting for last cmd to finish skip this sample
      Serial.println(" readHoldingRegisters skipped because modbus still busy with last cmd.");
    }
  }

  if (nbModbusMaster.justFinished()) {
    static int retryCount = 0;
    Serial.print(" cmd took "); Serial.print(millis() - start_ms); Serial.print(" ms.   ");
    // check for errors
